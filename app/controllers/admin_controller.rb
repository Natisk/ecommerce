...
class AdminController < ApplicationController
  ...
  def index
    ...
    todays_orders = ActiveRecord::Base.connection.execute(query)

    @dashboard_data = {
      revenue: todays_orders.first['total_sum'],
      orders_count: todays_orders.first['orders_count'],
      avg_sale: todays_orders.first['total_average'],
      per_sale: todays_orders.first['order_products_count']
    }
